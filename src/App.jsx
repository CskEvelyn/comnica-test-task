import {
  BrowserRouter as Router,
  Routes,
  Route,
  Link,
  useLocation,
} from "react-router-dom";
import FirstTask from "./components/pages/FirstTask";
import SecondTask from "./components/pages/SecondTask";
import ThirdTask from "./components/pages/ThirdTask";
import FourthTask from "./components/pages/FourthTask";

function Navigation() {
  const isHomePage = useLocation().pathname === "/";
  return (
    <nav className="">
      {isHomePage ? (
        <ul className="flex h-screen flex-col items-center justify-center px-4 text-sm uppercase text-white">
          <li>
            <Link
              className="mb-10 flex w-72 items-center justify-center rounded-md bg-green-main p-4 transition-all duration-300 hover:bg-green-dark"
              to="/first-task"
            >
              I. feladat
            </Link>
          </li>
          <li>
            <Link
              className="mb-10 flex w-72 items-center justify-center rounded-md bg-green-main p-4 transition-all duration-300 hover:bg-green-dark"
              to="/second-task"
            >
              II. feladat
            </Link>
          </li>
          <li>
            <Link
              className="mb-10 flex w-72 items-center justify-center rounded-md bg-green-main p-4 transition-all duration-300 hover:bg-green-dark"
              to="/third-task"
            >
              III. feladat
            </Link>
          </li>
          <li>
            <Link
              className="flex w-72 items-center justify-center rounded-md bg-green-main p-4 transition-all duration-300 hover:bg-green-dark"
              to="/fourth-task"
            >
              IV. feladat
            </Link>
          </li>
        </ul>
      ) : (
        <Link
          className="fixed bottom-10 right-10 z-20 rounded-full bg-purple/70 p-3"
          to="/"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            id="home"
          >
            <path
              fill="none"
              stroke="white"
              d="M6.65721519,18.7714023 L6.65721519,15.70467 C6.65719744,14.9246392 7.29311743,14.2908272 8.08101266,14.2855921 L10.9670886,14.2855921 C11.7587434,14.2855921 12.4005063,14.9209349 12.4005063,15.70467 L12.4005063,15.70467 L12.4005063,18.7809263 C12.4003226,19.4432001 12.9342557,19.984478 13.603038,20 L15.5270886,20 C17.4451246,20 19,18.4606794 19,16.5618312 L19,16.5618312 L19,7.8378351 C18.9897577,7.09082692 18.6354747,6.38934919 18.0379747,5.93303245 L11.4577215,0.685301154 C10.3049347,-0.228433718 8.66620456,-0.228433718 7.51341772,0.685301154 L0.962025316,5.94255646 C0.362258604,6.39702249 0.00738668938,7.09966612 0,7.84735911 L0,16.5618312 C0,18.4606794 1.55487539,20 3.47291139,20 L5.39696203,20 C6.08235439,20 6.63797468,19.4499381 6.63797468,18.7714023 L6.63797468,18.7714023"
              transform="translate(2.5 2)"
            ></path>
          </svg>
        </Link>
      )}
    </nav>
  );
}

function App() {
  return (
    <Router>
      <div className="App">
        <Navigation />
        <Routes>
          <Route path="/" element={<Navigation />} />
          <Route path="/first-task" element={<FirstTask />} />
          <Route path="/second-task" element={<SecondTask />} />
          <Route path="/third-task" element={<ThirdTask />} />
          <Route path="/fourth-task" element={<FourthTask />} />
        </Routes>
      </div>
    </Router>
  );
}

export default App;
